...
import static com.code.files.utils.Constants.CATEGORY_TYPE;
import static com.code.files.utils.Constants.CONTENT_ID;
import static com.code.files.utils.Constants.CONTENT_TITLE;
import static com.code.files.utils.Constants.IMAGE_URL;
import static com.code.files.utils.Constants.IS_FROM_CONTINUE_WATCHING;
import static com.code.files.utils.Constants.POSITION;
import static com.code.files.utils.Constants.SERVER_TYPE;
import static com.code.files.utils.Constants.STREAM_URL;
import static com.code.files.utils.Constants.YOUTUBE;
import static com.code.files.utils.Constants.YOUTUBE_LIVE;
...
public class DetailsActivity extends AppCompatActivity implements CastPlayer.SessionAvailabilityListener, ProgramAdapter.OnProgramClickListener, EpisodeAdapter.OnTVSeriesEpisodeItemClickListener,
        RelatedTvAdapter.RelatedTvClickListener, SubscriptionDialog.OnAdLoadingCallback {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        configViewModel = new ViewModelProvider(DetailsActivity.this).get(ConfigViewModel.class);
        ...
        fromAds = getIntent().getBooleanExtra("fromAds", false);
        ...
        subscriptionDialog = new SubscriptionDialog(DetailsActivity.this, configViewModel);
                    ...
                    if (listServer.size() == 1) {
                        if (isPaid) {
                            if (PreferenceUtils.isActivePlan(DetailsActivity.this)) {
                                playMovie(listServer.get(0));
                            } else {
                                selectedObj = listServer.get(0);
    ...
    private void playMovie(CommonModels model) {
        releasePlayer();
        //resetCastPlayer();
        preparePlayer(model);
        descriptionLayout.setVisibility(GONE);
    ...
    private void openServerDialog() {
        ...
        serverAdapter.setOnItemClickListener(new ServerAdapter.OnItemClickListener() {
            ...
            public void onItemClick(View view, CommonModels obj, int position, ServerAdapter.OriginalViewHolder holder) {
                selectedObj = obj;
                if (isPaid && !PreferenceUtils.isActivePlan(DetailsActivity.this)) {
                    fromAds = false;
                    subscriptionDialog.showDialog();
                } else {
                    playMovie(selectedObj);
                }
    ...
    private void loadAd() {
        ConfigViewModel configViewModel = new ViewModelProvider(DetailsActivity.this).get(ConfigViewModel.class);
        new BannerAds(this, this, adView, configViewModel).showBannerAds();
        new PopUpAds(this, DetailsActivity.this, configViewModel).showInterstitialAd();
    }
    ...
    private void initGetData() {
        ...
        if (vpnStatus) {
            ...
            if (!categoryType.equals("tv")) {
                ...
                if (!PreferenceUtils.isProgramGuideEnabled(configViewModel)) {
                ...
                relatedTvAdapter = new RelatedTvAdapter(listRelated, DetailsActivity.this, PreferenceUtils.isMandatoryLogin(configViewModel));
    ...
    public void initMoviePlayer(String url, String type, Context context) {
        serverType = type;
        urlType = type;
        if (type.equals("embed") || type.equals("gdrive") /*|| type.equalsIgnoreCase("youtube-live")*/) {
            isVideo = false;
    ...
    public void initVideoPlayer(String url, Context context, String type) {
        ...
        switch (type) {
            ...
            case YOUTUBE_LIVE:
                ...
                //go to youtube player activity
                Intent youtubeIntent = new Intent(DetailsActivity.this, YoutubeActivity.class);
            ...
            case "rtmp":
                ...
                break;
            default:
                Log.e(TAG, "initVideoPlayer: link: " + url );
                Log.e(TAG, "initVideoPlayer: type: " + type );
                new ParseStreamLink(DetailsActivity.this, url, type, new LinkParserCallback() {
                    @Override
                    public void onSuccess(List<Stream> result) {
                        String extension = result.get(0).getExtension();
                        if (extension.equalsIgnoreCase("m3u8")) {
                            mediaSource = hlsMediaSource(Uri.parse(result.get(0).getUrl()), context);
                        } else {
                            mediaSource = mediaSource(Uri.parse(result.get(0).getUrl()), context);
                        }
                        play(mediaSource);
                    }
                    ...
                    public void onError(String error) {
                        ...
                        new ToastMsg(DetailsActivity.this).toastIconError(error);
    ...
    private MediaSource hlsMediaSource(Uri uri, Context context) {
        ExtractorsFactory extractorsFactory = new DefaultExtractorsFactory().setTsExtractorFlags(FLAG_DETECT_ACCESS_UNITS).setTsExtractorFlags(FLAG_ALLOW_NON_IDR_KEYFRAMES);
            ...
            public void onResponse(Call<SingleDetailsTV> call, retrofit2.Response<SingleDetailsTV> response) {
                if (response.code() == 200) {
                    if (response.body() != null) {
                        ...
                        if (detailsModel.getStreamFrom().equalsIgnoreCase("youtube") || detailsModel.getStreamFrom().equalsIgnoreCase("youtube_live")) {
                            Intent intent = new Intent(DetailsActivity.this, YoutubeActivity.class);
                            intent.putExtra("url", detailsModel.getStreamUrl());
                            startActivity(intent);
                        } else {
        ...
        call.enqueue(new Callback<SingleDetails>() {
            ...
            public void onResponse(Call<SingleDetails> call, retrofit2.Response<SingleDetails> response) {
                if (response.code() == 200) {
                    ...
                    SingleDetails singleDetails = response.body();
                    isPaid = singleDetails.getIsPaid().equalsIgnoreCase("1");
                    //TODO: Paid control removed for testing.
